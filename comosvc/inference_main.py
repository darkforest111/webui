...
def main():
    ...
    for clean_name, tran in zip(clean_names, keys):
        ...
        for spk in spk_list:
            ...
            basename= os.path.splitext(os.path.basename(diffusion_model_path))[0]
            step_num = basename.split('.')[0]
            if args.teacher:
                isdiffusion = "teacher"
            else:
                isdiffusion= "como"
            res_path = os.path.join("../", resultfolder, f"{clean_name}_{spk}_{isdiffusion}_{step_num}.{wav_format}")
